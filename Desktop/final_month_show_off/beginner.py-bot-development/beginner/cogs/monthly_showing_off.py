...
class MonthlyShowingOffCog(Cog):
    ...
    async def github_response(self, message, json, author_id, time_sent):
        ...
        if size == 0:
            await message.channel.send(embed = discord.Embed(title = "Error!", description = f"{message.author.mention} Thats an empty Repository!", color = discord.Colour.red()), delete_after = 8)
            await message.delete()
            return 
        ...
        git_embed = self.parse_git_to_embed(project_name, owner, avatar, profile_url, description, project_url, language, message)
    ...
    async def get_message_history(self):
        ...
        users = []
        channel = self.client.get_channel(836419179779063868) 
        this_month = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        last_month = (this_month - timedelta(days=1)).replace(day=1)
        messages = await channel.history(after = last_month, before = this_month, limit = 1000).flatten()
    ...
    async def get_multiple_winners(self, winner_ids):
        ...
        try:    # Getting winner objects with a try block just in case the db doesnt have the person
            winner_details = [self.channel.guild.get_member(self.get_author_id(id)) for id in winner_ids]
        ...
        for id in winner_ids:
            ...
            if embed.fields:
                winner_projects.append([embed.fields[2].value, embed.fields[3].value][len(embed.fields) == 5])
            else: 
    ...
    async def get_single_winner_info(self, message_id):
        ...
        try:
            ...
            winner_project_url = [winner_project_details.fields[2].value, winner_project_details.fields[3].value][len(winner_project_details.fields) == 5] if bool(len(winner_project_details.fields)) else winner_project_details.description[9:]
    ...
    async def get_winners(self, votes, users):
        ...
        # If there is more than one winner
            ...
            await self.channel.send(embed =  await self.get_multiple_winners(winners))  # Like i did here
